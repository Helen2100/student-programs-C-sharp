...
namespace Manipulation
{
    public static class AnglesToCoordinatesTask
    {
        ...
        {//отправить
            ...
            x = PointX(Math.Abs(shoulder), x, Manipulator.UpperArm);
            y = PointY(Math.Abs(shoulder), y, Manipulator.UpperArm);
            ...
            x = PointX(Math.Abs(wrist + shoulder + elbow + 2 * Math.PI), x, Manipulator.Palm); ;
            y = PointY(Math.Abs(wrist + shoulder + elbow + 2 * Math.PI), y, Manipulator.Palm);
        ...
        {
            var sin = Math.Round(Math.Sin(corner));
